...
public class Notifications {
	...
	public void restaurantdetails(String region) {
		...
		for (int i = 0; i < restaurntDetails.size(); i++) {
			...
			System.out.println((i + 1 + "." + restaurntDetails.get(i).getRestaurantId()+ restaurntDetails.get(i).getRestaurantName()));
	...
	private void offerdetails(Long restaurantid) {
		...
		for(int i=0;i<offerdetails.size(); i++) {
			...
			System.out.println(offerdetails.get(i).getOfferId() + offerdetails.get(i).getOfferDescription() + offerdetails.get(i).getOfferPercentage());
